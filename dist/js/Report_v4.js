...
function SetCompletedTradeDetails(item) {
    ...
    for (var i = 0; i < ctable.rows.length; i++) {
        var Status = $(ctable.rows[i].cells[5]).text();
        if (Status == "TARGET" || Status == "TARGET2" || Status == "TARGET3") {
            $(ctable.rows[i].cells[5]).css("background-color", "dodgerblue");
            $(ctable.rows[i].cells[5]).css("color", "white");
        }
        if (Status == "STOPLOSS") {
            $(ctable.rows[i].cells[5]).css("background-color", "#ff4500b5");
            $(ctable.rows[i].cells[5]).css("color", "white");
        }
        var ProfitLoss = $(ctable.rows[i].cells[12]).text();
        if (ProfitLoss >= 0) {
            $(ctable.rows[i].cells[12]).css({ "color": "#14a964", "font-weight": "bold" });
        }
        if (ProfitLoss < 0) {
            $(ctable.rows[i].cells[12]).css({ "color": "#d83824", "font-weight": "bold" });
        }
