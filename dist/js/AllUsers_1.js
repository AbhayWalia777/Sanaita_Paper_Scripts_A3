...
function SetAllUsersDetails(item) {
    ...
    if (RoleId == 5 || RoleId == 4) {
        ...
        DeleteAction += `<button onclick="if(confirm('All data associated with this user will be permanently deleted and cannot be recovered. Are you sure you want to continue?')) window.location.href='/Admin/ResetUser?UserID=${item.UserID}&EmailID=${item.Email}'" type="button" class="btn btn-warning btn-sm ml-2"><i class="fa fa-refresh"></i></button>`;
    }
    if (RoleId == 5)
